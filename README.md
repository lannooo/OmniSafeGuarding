...
* Benchmark preparation guide: [BENCHMARKS.md](./BENCHMARKS.md)
* Guardrail finetuning guide: [MODELS.md](./MODELS.md)
...
```bash
# Prerequisite: conda, Python 3.10, PyTorch 2.10, Transformers >= 5.2, and vLLM.
bash env_setup.sh
...
Before prepare: login to Hugging Face and generate an access token, and login to ensure the required access permissions:
...
The following mini datasets may be missing in anonymous repo releases (too large, managed via Git LFS). They are disabled in default evaluation configs; after preparing local data, uncomment related eval sets:

- `data/external/JailBreakV-28k-mini.zip`
- `data/external/MML-SafeBench.zip`
- `data/external/Omniguard_Custom_mini.zip`
- `data/externalSafeWatch-Bench-mini.zip`

THe Omniguard_Custom_xx.zip is used for evaluating **Modality Bias** issue. Download full dataset from anonymous link: [Zenodo](https://zenodo.org/records/22157542). Place the full version `Omniguard_Custom_v4.zip` under `data/external/`.

For other dataset-specific issues and access reminders (HF terms/login), see [BENCHMARKS.md](BENCHMARKS.md).

### Prepare Base Models [Required]

Model training and inference both require base models. By default, models are saved into `models/base`, and with only `Qwen/Qwen2.5-Omni-7B` be downloaded, feel free to uncomment other model options in `prepare_base_models_hf.sh`. (Since these model checkpoints are large, make sure you have sufficient free disk space)
...
This is a key sanity-check step for the full pipeline. Running this script verifies that your environment and base model setup are working correctly. The script loads a base Omni model and runs some test cases for either multimodal QA or zero-shot guardrail inference.
...
We group all the datasets into different category, for evaluating

- safety: includes safety-I (basic safety evaluation) and safety-II (jailbreak defense evaluation), evaluates detection performance on malicious queries
- utility: evaluates interference on safe queries
- modality bias (mb): evaluates cross-modal modality bias issue
...
Training your own guardrail within three steps. See guide to [MODELS.md](MODELS.md).
...
**Notes**: Multimodal training may cause OOM (ensure that GPU has memory more than 40GB). If that happens, adjust settings in the finetuning script (more visible GPUs and/or reduce batch size).
...
Use this path when you want full end-to-end reproducibility from model outputs (it will take more time). Use `eval_omniguard_sft_all.py` or `eval_omniguard_sft.py`. This produces prediction outputs and logs that can be used for metric calculation and report generation.

### Path 2: Reproduce from Intermediate Results
...
- **Dataset/Training Data**: The related datasets/resources are intended for academic research only, under CC BY-NC 4.0.
